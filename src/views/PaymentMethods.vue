    ...
    <v-row>
      <v-col cols="12">
        <v-btn
          ...
          @click="router.push('/admin/payment/new')"
        >
          Add New
        </v-btn>
      </v-col>
...
const headers = [
  ...
  { title: "Payment Type", key: "name", sortable: true },
...
const fetchData = async (options = {}) => {
  ...
  try {
    const { data } = await api.get("auth/admin/payments", {
      params: {
        page: page || 1,
...
const confirmDelete = async () => {
  ...
  try {
    const res = await api.delete(
      `auth/admin/payments/${selectedItem.value.id}`
    );
